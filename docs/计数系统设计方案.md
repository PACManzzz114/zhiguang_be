...
- 写路径（状态事实为位图，增量事件异步折叠到汇总）：
  ...
  - Kafka 按实体维度设置稳定 message key；Producer 重试耗尽后写入指标级脏标记并废弃 SDS 快照，下次读取按 Bitmap 事实重建。
- 读路径（优先读汇总，异常时回溯事实）：
  - 常规读：直接读取 SDS 固定结构计数
  - 异常读：SDS 缺失、结构异常或存在指标级脏标记时，用位图分片 `BITCOUNT` 真实重建；分布式锁保护并清理对应聚合字段，避免重复加算。
...
- 实体计数（内容维度）
  ...
  - 投递失败脏标记：`dirty:cnt:{schema}:{metric}:{etype}:{eid}`，默认保留 24 小时，成功重建后删除。
  - 重建锁：`lock:sds-rebuild:{etype}:{eid}`，使用 Redisson 看门狗续约防止并发重建。
- 用户计数（用户维度，SDS）
...
- 位图切换（幂等原子）：
  ...
  - Kafka Producer 使用 `acks=all`、幂等生产与 3 次配置重试；异步 Future 最终失败时，记录实体/指标/用户/delta，写脏标记并删除该实体 SDS。
- 聚合增量消费：
...
- 灾备全量回放（可选）：
  - `counter.rebuild.enabled=true` 时，可使用 earliest 回放已经成功进入 Kafka 的历史事件（`CounterRebuildConsumer.java:15-21,35-53`）。未进入 Kafka 的失败事件不在回放范围内，由 Bitmap + 脏标记重建路径纠偏。

## 5. 读路径详解
- 常规读取（单 ID）：
  - `GET cnt:{schema}:{etype}:{eid}`，按照 Schema 偏移读取段值（大端 32 位），O(1)；若结构匹配则直接返回（`CounterServiceImpl.java:115-125,212-226`）。
- 异常重建：
  - SDS 缺失、长度异常或请求指标存在脏标记时，先尝试获取分布式锁；通过 `BITCOUNT bm:{metric}:{etype}:{eid}:*`（管道批量）求和，回写 SDS，清理对应聚合字段和脏标记。
- 批量读取（Feed 场景）：
...
- 最终一致：
  - 写入至聚合桶后至刷写的窗口为秒级（`fixedDelay=1000ms`），在窗口内读可能略滞后；投递失败时通过指标级脏标记触发事实重建。
- 原子折叠与上下限：
...
- 灾备：
  - 可切换重建消费者回放 Kafka 已落盘事件；Kafka 生产端开启幂等与严格确认，未落盘事件则依赖 Bitmap 事实重建。
...
- 投递补偿边界：当前是“Kafka 内部有限重试 + Redis 脏标记 + 读时 Bitmap 重建”，不是事务型消息表；若进程在 Bitmap 变更后、注册发送结果补偿前崩溃，或 Kafka 与 Redis 同时不可用，仍需依赖告警和后续对账。更强保证需将 Bitmap 变更与 Redis Stream/可重放事件记录绑定。
