...
- 说明：
  ...
  - 一致性：计数为最终一致。若目标计数快照缺失、结构异常，或 Kafka 事件最终投递失败后留下脏标记，服务端会在读取时触发位图扫描重建，并以分布式锁保护并发重建，完成后写回快照。
...
- 读取计数时，如快照缺失、长度异常或请求指标存在投递失败脏标记，会触发“位图扫描重建”，并使用分布式锁避免并发重建，重建完成后写回快照并返回结果。
